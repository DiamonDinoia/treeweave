...
input yields wrong values. When in doubt, use the plain batch path — it sorts
internally and is always correct. Both NaN-fill out-of-domain points.
...
Cross-language Mevals/s is **not** comparable (different CI runners) — compare
each language's treeweave bar to its own native bar.
Charts are regenerated by the ``benchmark-showcase`` CI matrix.
