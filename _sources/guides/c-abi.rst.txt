...
Out-of-domain handling is asymmetric (the cost of a branchless hot path):
evaluating exactly at the upper corner ``b`` returns the boundary value, points
below ``a`` (and ``NaN`` inputs) yield ``NaN``, and points above ``b``
extrapolate from the last cell (the result may be non-finite).
