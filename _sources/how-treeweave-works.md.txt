...
a degree-8 polynomial evaluation.
...
- **Monomial basis at low degree.** Treeweave keeps the polynomial in the plain
  ...
  (degree ≈ 8, well under the `n ≲ 30` regime where the monomial Vandermonde
...
```
   1D Horner (K = 9):              2D Horner (K = 9):

   y = c[8]                        for j in 0..8:
   for k in 7..0:                      y_j = (1D Horner over axis 1)
       y = y * x + c[k]
                                   y = y_8
   8 FMAs total, single chain.     for j in 7..0:
                                       y = y * x_0 + y_j

                                   80 FMAs total, two nested chains.
```

For 3D the cost is `9^3 − 1 = 728` FMAs per evaluation. The shipped kernel
...
Phase-7 attribution at 3D deg=8, N = 10^6:
...
One fit, immutable `Function`, cheap evaluations forever — the "baobab tree
of life" naming.
...
call site. Supported values are any positive integer; 8 is the default
