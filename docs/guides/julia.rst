...
On first build the package downloads the matching ``libtreeweave_c`` from the
GitHub Release and caches it. Override with the ``TREEWEAVE_REPO`` environment
...
each output component in its own contiguous row. Out-of-domain points are
NaN-filled on every route. ``sorted`` is 1-D only.
