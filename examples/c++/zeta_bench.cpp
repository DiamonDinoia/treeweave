// zeta_bench.cpp — treeweave vs a fair brute-force Riemann-zeta evaluation.
// ζ(s) = Σ_k k^(-s) summed until the tail is negligible (relative 1e-10, ≤160
// terms) is still expensive per eval yet smooth on [2,10]: fit once, eval a
// cheap polynomial. The native baseline is that same sum, so a fitted eval wins
// in every mode. Times single/multi/sorted; the native rate is sampled over
// n_native and reported mode-independently. TREEWEAVE_BENCH_YAML=path emits YAML.
// C++ member of the cross-language family (C, Fortran, Python, Julia, Octave, JS).
...
// A fair native baseline: accumulate k^(-s) until a term contributes less than
// kEps relative to the running total, capped at kMaxTerms. A competent
// hand-written zeta stops early once the tail is negligible — so this is an
// honest cost, not a fixed-iteration strawman. Used as both the fit callback
// and the baseline, so the comparison is apples-to-apples.
...
int main() {
    ...
    // --- sorted-eval: the 1-D ascending fast path (sort once, untimed) -----
