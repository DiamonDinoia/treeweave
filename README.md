...
evaluating at exactly `hi` returns `NaN` (the out-of-domain sentinel).
...
double y = fn(0.3);                              // single point

std::vector<double> xs{/* ... */}, ys(xs.size());
fn(xs.data(), ys.data(), xs.size());             // batch (any order)
fn.sorted(xs.data(), ys.data(), xs.size());      // 1-D ascending fast path
```
...
double xs[1000], ys[1000];             /* batch, array-of-structs     */
treeweave_batch(fn, xs, ys, 1000);     /* any order                   */
treeweave_sorted(fn, xs, ys, 1000);    /* 1-D ascending fast path      */
...
call treeweave_batch(h, xs, res, n)        ! batch, AoS
call treeweave_sorted(h, xs, res, n)       ! 1-D ascending fast path
h = treeweave_free(h)
...
approx(xs)                                           # batch
approx(xs, sorted=True)                              # 1-D ascending fast path
```
...
- **Python:** treeweave is not on PyPI yet — every push to `main` publishes a
  staging wheel to [TestPyPI](https://test.pypi.org/project/treeweave/). Install
  ...
  The x86-64 wheel dispatches SSE4.2 / AVX2 / AVX-512 at runtime; the C ABI is
  linked in statically. `pip install treeweave` will work once the first release
...
which charts each series over time and flags regressions. The exhaustive
exploratory history lives under [`bench/`](bench/).
...
- The fit covers the half-open interval `[x0, x1)`. Out-of-domain evaluation is
  best-effort, not an error: where it can do so without any out-of-bounds memory
  access (e.g. exactly at `x1` and just beyond) the nearest panel extrapolates a
  value with no accuracy guarantee; otherwise the result is `NaN`. No input
