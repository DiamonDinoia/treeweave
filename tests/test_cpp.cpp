...
TEST_CASE("Rejects non-positive tolerance", "[treeweave][errors]") {
    ...
    REQUIRE_THROWS_AS(fit(f, 0.0, 1.0, /*eps=*/0.0), std::invalid_argument);
    REQUIRE_THROWS_AS(fit(f, 0.0, 1.0, /*eps=*/-1.0), std::invalid_argument);
}
    ...
    auto run = [](auto fn, double a, double b) {
        ...
        for (std::size_t i = 0; i < N; ++i) {
            if (xs[i] < a) {
                ...
                // Finite high-OOD (accepted extrapolation region): the two batch
                // paths share the same leaf-id classification, so they still
                // agree — both clamp to the last leaf (a value) on the leaf-table
                // fast path, or both classify OOD -> NaN on the descent path.
                // The scalar operator() guard is inclusive-only (NaN here), so it
                // is *not* the oracle for this region.
                REQUIRE(std::isnan(sorted_out[i]) == std::isnan(batch_out[i]));
