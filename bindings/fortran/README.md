...
> system `/usr/bin/f95`) accepts the syntax without diagnostic but may produce
> wrong stop-code behaviour. Always use a modern compiler.
>
> **CMake footgun on RHEL/Flatiron.** `enable_language(Fortran)` picks the first
> `gfortran` (or `f95`) on `PATH`, which on these systems resolves to the system
> gfortran 8.5. Always pass the compiler explicitly at configure time:
>
> ```bash
> # Load a modern toolchain first (example: Flatiron Lmod)
> module load gcc/13.3.0       # or gcc/14.x
> gfortran --version           # verify: should be >= 10, ideally >= 13
>
> cmake -S . -B build \
>     -DTREEWEAVE_BUILD_FORTRAN=ON \
>     -DCMAKE_Fortran_COMPILER=$(which gfortran)
> ```
>
> Alternatively set `FC=$(which gfortran)` before running CMake. Without this
...
| `test_treeweave.f90` | Self-checking test (`error stop` on failure → nonzero exit); covers scalar-eval, batch, sorted, transposed (SoA), print_stats, f32, and context |
