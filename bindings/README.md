...
not `NaN`. The guard is asymmetric: points below `a` (and `NaN` inputs) return
`NaN`; far above `b`, batch eval extrapolates from the last cell. This keeps the
hot path branchless.
...
gateway (`treeweave_mex_gen.cpp`) + `bz_*.m` stubs in the build tree, and
