...
export class Treeweave {
  ...
  static async fit(
    ...
    if (dim !== aArr.length) throw new Error(`dim=${dim} but len(a)=${aArr.length}; they must agree`);

    const dtype: DType = opts.dtype ?? "f64";
    const make = dtype === "f32" ? Float32Array.from.bind(Float32Array) : Float64Array.from.bind(Float64Array);
  ...
  sorted(x: number[] | FloatArray, opts: { out?: FloatArray } = {}): FloatArray {
    if (this.dim !== 1) throw new Error(`sorted requires dim == 1; this fit has dim=${this.dim}`);
