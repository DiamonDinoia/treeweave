...
test("at least one backend was built", () => {
  assert.ok(backends.length > 0, "no backend artifacts found in dist/ (build bindings-js and/or bindings-js-wasm)");
});
...
for (const backend of backends) {
  describe(`treeweave [${backend}]`, () => {
    ...
    test("out= writes in place, is returned as-is and bit-exact", async () => {
      ...
      assert.deepEqual(Array.from(buf), Array.from(fresh), "out= must be bit-exact vs a fresh result");
      tw.free();
    });

    test("transposed returns one array per output component", async () => {
      const tw = await Treeweave.fit((x) => [zeta(x[0]), Math.log(x[0])], A, B, TOL, { backend });
    ...
    test("error paths: bad point length, bad out= length", async () => {
      ...
      assert.throws(() => tw.batch(Float64Array.of(5, 6, 7), { out: new Float64Array(2) }), /elements/);
