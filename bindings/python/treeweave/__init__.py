...
class TreeweaveFunction:
    ...
    def __repr__(self) -> str:
        """One-line summary of the fitted approximation."""
        return (
            f"{type(self).__name__}(dtype={self.dtype!r}, dim={self.dim}, "
            f"out_dim={self.out_dim}, memory_usage={self.memory_usage} B)"
        )
    ...
    def __call__(self, x, *, sorted: bool = False, transposed: bool = False):
        ...
        """
        if sorted and transposed:
            raise ValueError("sorted=True and transposed=True are mutually exclusive")

        dtype = np.float32 if self.dtype == "f32" else np.float64
        ...
        if transposed:
            ...
            return np.stack(soa, axis=0)  # (out_dim, N)
        ...
        if x.ndim == 1:
            if self.dim == 1:
                ...
                return self._inner.eval_multi(np.ascontiguousarray(x))
    ...
    def _coerce_batch(self, x: np.ndarray) -> np.ndarray:
...
def fit(
    ...
    *,
    ...
    if out_dim is None:
        ...
        probe = f(midpoint)
        out_dim = int(np.asarray(probe).size)
