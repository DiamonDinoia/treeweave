 ...
 * @param f Handle from treeweave_fit().
 * @param x Input coordinates, @c input_dim elements.
 * @param y Output buffer, @c output_dim elements. Out-of-domain inputs yield NaN.
 */
 ...
 * vector output keeps the pointer API. On a dimension mismatch (the handle's
 * input_dim/output_dim does not match this arity / scalar-output) they set
 * treeweave_last_error() and return NaN.
 *
...
/** @brief Approximate bytes held by the approximation tree. */
TREEWEAVE_EXPORT size_t treeweave_memory_usage(treeweave_t f);
/** @brief Print internal fit/eval statistics to stdout. */
