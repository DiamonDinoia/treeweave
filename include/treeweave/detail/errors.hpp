...
class MaxDepthExceeded : public std::exception {
  public:
    ...
    explicit MaxDepthExceeded(std::vector<NonConvergedPanel> panels) : panels_(std::move(panels)) {
        if (!panels_.empty()) {
        ...
        os << "Treeweave fit error: tree depth exceeded max allowed input depth (" << depth_ << ") on "
           << panels_.size() << " panel" << (panels_.size() == 1 ? "" : "s") << "; first ";
